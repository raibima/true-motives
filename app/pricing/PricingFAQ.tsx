...
export function PricingFAQ() {
  return (
    <AriaDisclosureGroup className="space-y-0 divide-y divide-(--tm-color-neutral-100) rounded-xl border border-(--tm-color-neutral-100) overflow-hidden">
      {FAQ_ITEMS.map((item) => (
        <AriaDisclosure
          key={item.id}
          id={item.id}
        ...
        >
          ...
          <AriaDisclosurePanel className="h-(--disclosure-panel-height) motion-safe:transition-[height] overflow-clip">
            <div className="px-5 pb-5 pt-0 text-sm leading-relaxed text-(--tm-color-neutral-600)">
              {item.answer}
            </div>
          </AriaDisclosurePanel>
        </AriaDisclosure>
...
function FAQTrigger({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Heading className="m-0">
      <Button
        slot="trigger"
        className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left text-sm font-semibold text-(--tm-color-primary-900) transition-colors hover:bg-(--tm-color-neutral-50) focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-(--tm-color-accent-400) cursor-default"
      >
        <span>{children}</span>
        <svg
