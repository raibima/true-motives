  ...
  if (isMockInvestigation) {
    ...
    if (runStatus === 'completed') {
      ...
      investigation = {
        id,
        title: `Investigation ${id}`,
        description: 'Deep research is currently running. Live progress will stream below.',
        status: 'generating',
        category: 'policy',
        geography: 'Global',
        createdAt,
    ...
    <div className="max-w-3xl px-8 py-8">
      ...
      <div className="animate-fade-in-up mb-8">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <CategoryBadge category={investigation.category} className="rounded-md px-2 py-1" />
        ...
        <h1 className="mb-2 font-serif text-2xl leading-snug font-semibold text-(--tm-color-primary-900)">
          {investigation.title}
        </h1>

        <p className="mb-3 text-sm leading-relaxed text-(--tm-color-neutral-600)">
      ...
      {investigation.status === 'failed' && (
        <div className="animate-fade-in-up stagger-2 rounded-xl border border-(--tm-color-danger-500)/30 bg-(--tm-color-danger-100)/60 p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="mb-1 text-sm font-semibold text-(--tm-color-danger-500)">
                Generation failed
              </h2>
              <p className="mb-4 text-sm text-(--tm-color-danger-500)/80">
                Something went wrong during deep research. Your investigation has been saved and you
                can try generating it again.
              </p>
      ...
      {investigation.status === 'draft' && (
        <PlaceholderCard
          ...
          action={
            <Button variant="primary" className="h-auto px-5 py-2.5 font-semibold">
      ...
      {investigation.status === 'completed' && investigation.report && (
        <div className="animate-fade-in-up stagger-2 space-y-6">
          ...
          <div className="flex items-start gap-3 rounded-xl border border-(--tm-color-neutral-100) bg-(--tm-color-neutral-50) px-4 py-3">
            ...
            <p className="text-xs leading-relaxed text-(--tm-color-neutral-600)">
              This is an AI-assisted motivation analysis. All claims are hypothesis-grade unless
              explicitly marked as verified facts. Always corroborate with primary sources before
              publication.
            </p>
          </div>
          ...
          <ReportSection title="Executive summary">
            <div className="space-y-3 text-sm leading-7 text-(--tm-color-neutral-600)">
          ...
          <ReportSection title="Motivation hypotheses">
            <div className="space-y-5">
              {investigation.report.motivations.map((motivation, index) => (
                <div
                  key={index}
                  className="space-y-2 border-l-2 border-(--tm-color-accent-500)/30 pl-4"
                >
          ...
          <ReportSection title="Stakeholder map">
            <div className="space-y-4">
              {investigation.report.stakeholders.map((stakeholder, index) => (
                <div key={index} className="rounded-lg border border-(--tm-color-neutral-100) p-4">
