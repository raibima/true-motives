...
const tabListStyles = tv({
  ...
  variants: {
    ...
    variant: {
      pill: 'p-1 -m-1',
      underline: 'items-center gap-1 border-b border-(--tm-color-neutral-100) pb-0',
    },
  },
});

export interface TabListPropsWithVariant<T extends object = object> extends TabListProps<T> {
  variant?: TabVariant;
}
...
const selectionIndicatorStyles = tv({
  ...
  variants: {
    variant: {
      pill: 'absolute top-0 left-0 w-full h-full z-10 bg-white rounded-full mix-blend-difference group-disabled:bg-neutral-400 group-disabled:mix-blend-normal group-disabled:dark:bg-neutral-600 group-disabled:-z-1',
      underline: 'absolute left-0 bottom-0 w-full h-0.5 bg-(--tm-color-primary-900)',
    },
  },
});
  ...
  return (
    ...
    >
      {composeRenderProps(props.children, (children) => (
        <>
          {children}
          <SelectionIndicator className={selectionIndicatorStyles({variant})} />
        </>
      ))}
    </RACTab>
  );
