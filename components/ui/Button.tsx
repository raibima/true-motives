...
let button = tv({
  ...
  variants: {
    variant: {
      primary: 'bg-(--tm-color-primary-900) hover:bg-(--tm-color-primary-800) pressed:bg-(--tm-color-primary-600) text-white shadow-sm',
      ...
      outline: 'border-(--tm-color-neutral-100) bg-white hover:bg-(--tm-color-neutral-50) pressed:bg-(--tm-color-neutral-100) text-(--tm-color-neutral-600) hover:text-(--tm-color-primary-900)',
      destructive: 'bg-(--tm-color-danger-500) hover:bg-(--tm-color-danger-500)/90 pressed:bg-(--tm-color-danger-600) text-white',
        ...
        <>
          ...
          {isPending && (
            <span aria-hidden className="flex absolute inset-0 justify-center items-center">
              <svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" stroke={props.variant === 'secondary' || props.variant === 'quiet' || props.variant === 'outline' ? 'light-dark(black, white)' : 'white'}>
