(() => {
  ...
  (() => {
    ...
    function checkAndHandleRestrictedPageIfSo(e) {
      e &&
        isRestrictedBrowserPage(e.url) &&
        background_setExtensionIconAndPopup('restricted', e.id);
    }
    (chrome.tabs.query({}, (e) => e.forEach(checkAndHandleRestrictedPageIfSo)),
      ...
      chrome.tabs.onUpdated.addListener((e, t, n) => {
        t.url &&
          isRestrictedBrowserPage(t.url) &&
          background_setExtensionIconAndPopup('restricted', e);
      }));
    ...
    function connectExtensionAndProxyPorts(e, n, r) {
      ...
      if (t[r].disconnectPipe)
        throw new Error(`Attempted to connect already connected ports for tab with id ${r}`);
      ...
      function disconnectListener() {
        (e.onMessage.removeListener(extensionPortMessageListener),
          ...
          delete t[r].disconnectPipe);
      }
      ((t[r].disconnectPipe = disconnectListener),
      ...
      chrome.runtime.onMessage.addListener((t, n) => {
        switch (t?.source) {
            ...
            !(function (e) {
              ...
              switch (t?.type) {
                case 'fetch-file-with-cache': {
                  ...
                  t && n
                    ? chrome.tabs.sendMessage(t, {
                        ...
                        payload: {type: 'fetch-file-with-cache', url: n},
                      })
                ...
                case 'eval-in-inspected-window': {
                  const {
                    payload: {tabId: t, requestId: n, scriptId: r, args: o},
                  } = e;
                  chrome.tabs
                    .sendMessage(t, {source: 'devtools-page-eval', payload: {scriptId: r, args: o}})
                    .then((e) => {
                      if (!e)
          ...
          case 'react-devtools-backend-manager':
            !(function (t, n) {
              const {payload: r} = t;
              'require-backends' === r?.type &&
                r.versions.forEach((t) => {
                  e.includes(t) &&
                    executeScriptInMainWorld({
                      injectImmediately: !0,
                      target: {tabId: n.tab.id},
                      files: [`/build/react_devtools_backend_${t}.js`],
                    });
                });
            })(t, n);
            break;
          case 'react-devtools-hook':
            !(function (e, t) {
              const {payload: n} = e;
              'react-renderer-attached' === n?.type &&
                background_setExtensionIconAndPopup(n.reactBuildType, t.tab.id);
            })(t, n);
        }
      }),
      chrome.tabs.onActivated.addListener(({tabId: e}) => {
        for (const n in t)
          if (null != t[n].proxy && null != t[n].extension) {
            const r = e === +n ? 'resumeElementPolling' : 'pauseElementPolling';
            t[n].extension.postMessage({event: r});
          }
      }));
  })();
})();
